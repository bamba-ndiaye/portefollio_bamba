...
export function ContactSection() {
  ...
  return (
    <section ref={sectionRef} id="contact" className="py-20 px-4 relative z-10">
      <div className="max-w-4xl mx-auto">
        ...
        <GlassCard
          className={`p-8 md:p-12 transition-all duration-700 delay-100 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          }`}
        >
          <div className="text-center mb-8">
            <h3 className="text-2xl font-semibold mb-4 text-foreground">Travaillons Ensemble</h3>
            <p className="text-muted-foreground leading-relaxed">
              Je suis actuellement disponible pour des opportunités freelance ou à temps plein. N'hésitez pas à me
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {contactLinks.map((link, index) => (
              <a
                ...
                className="flex items-center gap-4 p-4 rounded-lg border border-accent/20 bg-card/20 hover:bg-accent/10 hover:border-accent/40 transition-all group"
              >
                <div className="text-accent group-hover:scale-110 transition-transform">{link.icon}</div>
          ...
          <div className="mt-8 pt-8 border-t border-accent/20 text-center">
            <p className="text-muted-foreground text-sm">📍 {portfolioData.contact.location}</p>
          </div>
        </GlassCard>
      </div>
    </section>
